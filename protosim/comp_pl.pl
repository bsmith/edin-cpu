...
# tools for the DSL
# TODO: use Sub::Quote here!
our @reset_latches;
sub on_reset_latches {
  push @reset_latches, $_[0];
}

sub call_reset_latches {
  $_->() for @reset_latches;
}
...
# Loading the net definition file

{ # XXX do is fucking crazy do not use
  local ($@, $!);
  my $rv = do './fibonacci.pl';
  if (not defined $rv) {
    if ($@) {
      warn "fibonacci.pl: parse fail\n";
      die $@;
    } else {
      die "fibonacci.pl: do failed: $!\n";
    }
  }
  if (not $rv) {
    die "fibonacci.pl: did not return true\n";
  }
}

timed_log "Defined nets from fibonacci.pl";
